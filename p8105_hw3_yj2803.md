...
df_brfss_cleaned <- brfss_smart2010 |> 
  janitor::clean_names() |> 
  filter(topic == "Overall Health" & response == c("Excellent", "Very good", "Good", "Fair", "Poor")) |>  # focus on Overall Health; only include responses from "Excellent" to "Poor"
  mutate(
...
df_brfss_2002 <- df_brfss_cleaned |> 
  ...
  summarise(count = n()) |> # count number of observations in each state
...
7 or more locations were observed in GA, MA, ME, NJ, OH, PA, RI in 2002;
CA, CO, FL, ID, MA, MD, ME, NC, NY, OH, PA, SC, TN, TX, UT in 2010.
...
![](p8105_hw3_yj2803_files/figure-gfm/unnamed-chunk-14-2.png)<!-- -->
